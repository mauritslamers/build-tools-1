...
exports.App = Framework.extend({
  ...
  addFramework: function(fw){
    var optslist, k;
    if(!fw) return;
    optslist = "combineStylesheets combineScripts minifyScripts minifyStylesheets server buildVersion isBundle bundleDeps shouldPreload".w();
  ...
  _createBundleFile: function(bundles){
    ...
    file = File.create({ 
      ...
      framework: this, 
      content: function(callback){ callback(null, content);}.property(), 
  ...
  _performSave: function(){
    var me = this,
        ...
        bundles = {},
        mainBundle = Bundle.create({ name: this.name, savePath: this.savePath });
    ...
    this._frameworks.forEach(function(fw){
      ...
      else mainBundle.add(fw.orderedScripts, fw.orderedStylesheets, fw.resources); 
    });
    
    // before saving the main bundle, we have to add the bundle info up front
    mainBundle.orderedScripts.unshift(this._createBundleInfo(bundles));
    mainBundle.save();
    ...
    html.content = this.rootContent(
      ...
      '<script type="text/javascript" src="' + this.urlPrefix + mainBundle.get('url') + '.js"></script>'
    );
  ...
  save: function(){
    if(!this._doneBuilding){
      this.build(this._performSave);
    }
