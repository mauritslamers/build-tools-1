...
exports.App = Framework.extend({
  ...
  addFramework: function(fw){
    var optslist, k;
    if(!fw) return;
    optslist = "combineStylesheets combineScripts minifyScripts minifyStylesheets server buildVersion".w();
    optslist.forEach(function(opt){
      if(!fw[opt]) fw[opt] = this[opt]; // only copy if not defined
    },this);
    
    k = fw.hasNestedFrameworks? FrameworkBundler: Framework; 
  ...
  addSproutcore: function(opts){
    this._frameworks.push(Sproutcore.create({ server: this.server }));
