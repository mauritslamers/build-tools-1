...
App.prototype.build = function(callback) {
  var Builder = function(app, callback) {
    ...
    that.callbackIfDone = function() {
      if (callback && that.count <= 0 && !that.hasCalledCallback){ // hack to prevent having the callback called twice...
...
App.prototype.saveTwo = function(){
  ...
  for(fw_i=0;fw_i<frameworks.length;fw_i+=1){
    ...
    if(!bundles[cur_fw.name()] && cur_fw.isBundle && !cur_fw.shouldPreload) bundles[cur_fw.name()] = makeBundle();
    targetBundle = (cur_fw.isBundle && !cur_fw.shouldPreload)? bundles[cur_fw.name()]: mainBundle;
