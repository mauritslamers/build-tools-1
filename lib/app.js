  ...
  _performSave: function(){
    ...
    this._frameworks.forEach(function(fw){
      ...
      if(fw.isSC){ // sc should not be mixed in with the main bundle, but should be separate, and not in the bundle info
        _sc_ = Bundle.create({ name: scname, buildPath: this.buildPath, buildVersion: this.buildVersion, framework: this });
    ...
    html.content = this.rootContent(
      ...
      '<script type="text/javascript" src="' + this.get('urlPrefix') + mainBundle.get('url') + '.js"></script>'
    );
