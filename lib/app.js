...
exports.App = Framework.extend({
  ...
  build: function(callback){
    ...
    var codeTreeReady = function(){
      fileCount += 1;
      if(fileCount === me.server.files.length){
        me.server.codeTree = uglifyParser.parse(me.server._allCode);
        callback();
      }
    };
        
    var codeTreeBuilder = function(){
      me.server._allCode = "";
      var addCode = function(code){
        me.server._allCode += code;
        codeTreeReady();
      };
      me.server.files.forEach(function(f){
        f.content(addCode);
      });
    };   
