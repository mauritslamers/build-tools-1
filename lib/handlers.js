...
try {
  ...
  l.sys.puts("WARNING: 'less' could not be required.");
  l.sys.puts("         You won't be able to parse .less files.");
  l.sys.puts("         Install it with `npm install less`.");
}
    ...
    that.next.handle(file, request, function(response) {
      ...
      if (l.less && file.extname() === '.less') {        
        ...
        parser.parse(response.data, function(err, tree) {
          if (err) {
            l.sys.puts('ERROR: ' + err.message);
            console.log(err);
          } else {
            try {
              response.data = tree.toCSS();
            } catch (e) {
              l.sys.puts('ERROR: ' + e.message);
              console.log(e);
            }
          }
          callback(response);
