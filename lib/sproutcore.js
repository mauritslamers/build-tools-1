...
exports.Sproutcore = FrameworkBundler.extend({
  ...
  internalSproutcoreDirectory: function(){
    var version = this.get('version') || "1.9.1";
  ...
  installSproutcore: function(callback){
    ...
    var cmd, version = this.get('version');
    ...
    tools.puts("Installing Sproutcore version " + version);
    exec(cmd, function(err, stdout,stderr){
      if(err){
        tools.puts("Installing Sproutcore version " + version + " failed");
        tools.puts(err);
        tools.put(stdout);
        tools.put(stderr);
        callback(err);
  ...
  init: function(){
    ...
    var v = this.version? this.version.split("."): "1.9.1".split(".");
    if(!this.frameworkNames){
      if(parseInt(v[1],10) >= 5){ // above 1.5.0
        this.frameworkNames = "bootstrap jquery runtime core_foundation template_view datetime foundation routing statechart datastore ajax desktop".split(" ");
      }
      else this.frameworkNames = "bootstrap jquery runtime foundation datastore desktop animation testing".split(" ");      
    }
  ...
  build: function(callback){
    ...
    tools.fs.exists(this.path,function(sc_in_proj_exists){
      if(sc_in_proj_exists) continueBuild();
        ...
        tools.fs.exist(internalSCDir, function(sc_internal_exists){
