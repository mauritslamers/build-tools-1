  ...
  _scanPath: function(path){
    ...
    var f = function(err,stats){
      ...
      if(stats.isDirectory()){
        //tools.log('about to attach watcher to path: ' + path);
        if(fw.watchForChanges){
          if(!fw._watchers) fw._watchers = [];
          fw._watchers.push(tools.fs.watch(path,function(){ fw._changewatcher.apply(fw,arguments);}));          
        }
        me._handledir(path);
      }
      else {
