      ...
      projectConfigHasRead: function(err,content){
        ...
        if(err) project.set('error',err);
          ...
          if(!validated || validated.length > 0){
            project.set('error',errors.ValidationError.create({ 
    ...
    SETUP_PROJECT: SC.State.design({
      ...
      SETUP_APPS: SC.State.design({
        enterState: function(){
          ...
          if(!apps){ // for now an error, we can try to do autodetection later
            project.set('error',errors.NoAppsConfiguredError.create());
            this.gotoState('PROJECT_CONFIG_ERROR');
          } 
          else {
            // for each app, first try to find a config file.
            return SC.Async.perform('setupApps');
          }
        },
        
        setupApps: function(){
          var apps = project.getPath('config.apps');
          this._apps = Object.keys(apps); // save a list of names
          Object.keys(apps).forEach(function(app){
            // try to find and load config for app
            
          });
        },
        
        appDidComplete: function(){
