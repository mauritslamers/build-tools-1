...
exports.Framework = SC.Object.extend({
  ...
  shouldPreload: true,
  watchForChanges: true, // watch for changes by default
  ...
  _buildStylesheets: function(files){
    ...
    handlers.push('slices',['rewriteStatic', "url('%@')"], 'join'); // file is done by join if needed
  ...
  findResourceFor: function(filename){
    ...
    if(!r) return [];
    var fn = tools.path.basename(filename); // strip off anything that doesn't belong there...
    var ret = this.resources.filter(function(f){
      if(f.get('path').indexOf(fn) >= 0) return true;
    });
    //tools.log('found resources for filename: ' + filename + ": " + tools.inspect(ret));
    return ret;
  },
  ...
  _save: function(file,index,files){
    var me = this;
    //tools.log('saving file ' + file.get('savePath'));
