...
exports.Framework = SC.Object.extend({
  ...
  urlFor: function(path){
    ...
    // if(this.belongsTo){
    //   return this.belongsTo.get('name') + "/" +  this.nameFor(path);  
    // }
    // else this.nameFor(path);
    //return this.belongsTo.get('name');
    return tools.path.join(this.belongsTo.get('name'), this.nameFor(path));
  },
  
  savePathFor: function(path){
    return tools.path.join(this.get('savePath'), this.nameFor(path));
  },
  ...
  _save: function(file,index,files){
    ...
    file.handler.handle(file,null,function(r){
      ...
      if(r.data.length > 0){ // only save files with content
        tools.log('buildPath: ' + me.get('buildPath'));
        tools.log('buildVersion: ' + me.get('buildVersion'));
        tools.log('savePath: ' + me.get('savePath'));
        tools.log('file savePath: ' + file.get('savePath'));
        ...
        tools.fs.writeFile(path,r.data,function(err){
          if(err){
            //tools.log('file savePath: ' + file.get('savePath'));
            tools.log('Error while trying to save file ' + file.get('savePath') + ' to ' + path);
            tools.log('File: ' + tools.inspect(file));
            tools.log('buildPath: ' + me.get('buildPath'));
            tools.log('buildVersion: ' + me.get('buildVersion'));
            tools.log('savePath: ' + me.get('savePath'));
            throw err;
          } 
          tools.log('file ' + path + " saved");
        });
