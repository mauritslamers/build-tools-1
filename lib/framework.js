...
self.Framework = function(options) {
  ...
  if (options.pathsToExclude instanceof Array) {
    ...
    this.pathsToExclude.push(options.pathsToExclude);
  }
};
...
Framework.prototype.build = function(callback) {
  ...
  var buildStylesheets = function(files) {
    ...
    handlers.push('join', 'less', ['rewriteStatic', "url('%@')"], 'file');
...
Framework.prototype.bundleInfo = function() {  
  return [
    ";SC.BUNDLE_INFO['" + this.name() + "'] = {",
      "requires: [],", // FIXME: fill the requires array with the dependent frameworks 
      "scripts: [" + this.orderedScripts.map(function(script) { return "'" + script.url() + "'"; }).join(",") + "],",
      "styles: [" + this.orderedStylesheets.map(function(stylesheet) { return "'" + stylesheet.url() + "'"; }).join(",") + "],",
    "};"
  ].join('\n');
};
...
Framework.sproutcoreFrameworks = function(options) {
  ...
  if (this._sproutcoreFrameworks === undefined) {    
    ...
    //list = ['jquery', 'runtime', 'handlebars', 'core_foundation', 'datetime', 'foundation', 'datastore', 'desktop'];
    //list = ['jquery','runtime','handlebars', 'core_foundation','foundation', 'datastore', 'desktop', 'animation'];
