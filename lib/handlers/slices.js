...
exports.Slices = Handler.extend({
  ...
  create_slice: function(opts){
    ...
    var optfiles = this._fw.findResourceFor(filename);
    //tools.log('optional matches: ' + optfiles.getEach('path'));
    var ourdir = tools.path.dirname(this._file.get('path'));
    var files = optfiles.filter(function(f){
      ...
      if(f.get('path') === tools.path.join(ourdir,filename)) return true;
    });
    ...
    includedSlices.forEach(function(slicename){
      ...
      if(!skip.contains(cssname)){
        ...
        slice.file.handler.handle(slice.file,{},function(r){
          var dataurl = me.slice_image.call(me,slice,r.data,slicename);
          //if (slice["x2"] != null) {
  ...
  finish: function(request,r,callback){
    ...
    if(!c) callback(r);
    if(this._file.get('isThemeDefinition')){
