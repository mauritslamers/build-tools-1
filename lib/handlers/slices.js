...
exports.Slices = Handler.extend({
  ...
  gsub: function(source,regex,matcher,matchertarget,callback,callbacktarget){
    ...
    var report_creator = function(match,index){
      var reporter = function(newdata){
        ...
        count += 1;
        //tools.log('matcher reporting in: ' + count);
    ...
    while (source.length > 0){
      ...
      if(match){
        ...
        matchercalls.push(report_creator(match,result_index));
  ...
  create_slice: function(opts){
    ...
    if(!files || files.length === 0) throw new Error('filename not found for slice! ' + filename);
    opts.file = files[0];
  ...
  handle_slices: function(opts,callback){
    ...
    var adjustSlices = function(slicename){
  ...
  slice_rect: function(slice, image_width, image_height) {
    var bottom, height, left, rect, right, top, width;
    left = slice["left"];
    top = slice["top"];
    bottom = slice["bottom"];
    right = slice["right"];
    ...
    if (rect["left"] === 0 && rect["top"] === 0 && rect["width"] === image_width && rect["height"] === image_height) {
      return null;
    }
    return rect;
  },
