...
exports.Server = SC.Object.extend({
  ...
  init: function(){
    ...
    var me = this;
    if(this.loadSCTests){
      var testApp = require('./apps/testapp');
      this.addApp(testApp);
    }
    this.registerURL('sc/targets.json', File.create({
      path: 'sc/targets.json',
      isVirtual: true,
      handler: HandlerSet.create({
        urlPrefix: "",
        handlerList: ['contentType','file']
      }),
      content: function(cb){
        me._renderTargets.call(me,cb);
      }
    }));
  },
  
  _renderTargets: function(cb){
    cb(null,JSON.stringify(this._targets));
  },
  ...
  unregisterURL: function(url,fileobj){
    if(!this.files) return;
    if(fileobj){
      if(this.files[url] === fileobj){
        this.files[url] = undefined;
      }
    }
    else {
      this.files[url] = undefined;
  ...
  */
  ...
  onRequest: function(request,response){
    ...
    //tools.log(Object.keys(this.files));
    if(file){
    ...
    tools.http.createServer(f).on('error',function(err){
      if(err){
        if(err.code === "EOF"){
          ...
          tools.log('Unknown error while trying to attach the server.');
          process.exit(1);
        }
      }  
      ...
      if(me.REPL){
        ...
        me.repl.start("scbt>>").context.server = me;
      }
    });
